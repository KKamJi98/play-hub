  ...
  return (
    <div className="flex flex-col items-center gap-8 py-8 px-4">
      ...
      <p className="text-[#8892a4] text-center max-w-md">
        8x8 보드 위에서 상대의 돌을 뒤집어 더 많은 돌을 차지하세요. 전략적으로 코너와 변을 장악하는 것이 승리의 열쇠입니다.
      </p>
...
function ScoreBoard({
  ...
  const blackLabel = mode === "ai" ? "당신 (흑)" : "Player 1 (흑)";
  const whiteLabel = mode === "ai" ? "AI (백)" : "Player 2 (백)";

  return (
    <div className="flex items-center justify-center gap-6 py-3">
      {/* Black player */}
      <div className={`flex items-center gap-2 ${currentPlayer === BLACK ? "opacity-100" : "opacity-50"}`}>
        <div
          className="w-5 h-5 rounded-full"
          style={{
            background: "radial-gradient(circle at 35% 35%, #444, #111 55%, #000)",
            boxShadow: "1px 1px 3px rgba(0,0,0,0.5)",
          }}
        />
        <span className="font-display text-sm font-semibold tracking-wide">{blackLabel}</span>
        <span className="font-display text-lg font-bold text-[#00f0ff]">{scores.black}</span>
      </div>

      <span className="text-[#8892a4] font-display text-sm">vs</span>

      {/* White player */}
      <div className={`flex items-center gap-2 ${currentPlayer === WHITE ? "opacity-100" : "opacity-50"}`}>
        <span className="font-display text-lg font-bold text-[#ffb800]">{scores.white}</span>
        <span className="font-display text-sm font-semibold tracking-wide">{whiteLabel}</span>
        <div
          className="w-5 h-5 rounded-full"
          style={{
            background: "radial-gradient(circle at 35% 35%, #fff, #f0f0f0 40%, #ddd 70%, #ccc)",
...
function GameOverModal({
  ...
  if (winner === 0) {
    title = "무승부!";
    subtitle = `양 플레이어가 동일한 ${scores.black}개의 돌을 가지고 있습니다.`;
  } else if (mode === "ai") {
    if (winner === BLACK) {
      title = "승리!";
      subtitle = `축하합니다! ${scores.black} 대 ${scores.white}로 AI를 이겼습니다.`;
    } else {
      title = "패배...";
      subtitle = `AI가 ${scores.white} 대 ${scores.black}로 승리했습니다. 다시 도전해보세요!`;
    }
  } else {
    title = winner === BLACK ? "흑 승리!" : "백 승리!";
    subtitle = `${scores.black} 대 ${scores.white}로 ${winner === BLACK ? "Player 1" : "Player 2"}이(가) 승리했습니다.`;
  }
        ...
        <div className="text-center">
          ...
          <div className="flex items-center justify-center gap-4 mt-4">
            <div className="flex items-center gap-1.5">
              ...
              />
              <span className="font-display text-lg font-bold">{scores.black}</span>
            </div>
            <span className="text-[#8892a4]">-</span>
            <div className="flex items-center gap-1.5">
              <span className="font-display text-lg font-bold">{scores.white}</span>
              <div
                className="w-4 h-4 rounded-full"
                style={{
                  background: "radial-gradient(circle at 35% 35%, #fff, #f0f0f0 40%, #ddd 70%, #ccc)",
          ...
          >
            ...
            <div className="rounded-2xl border border-white/10 bg-black/10 px-4 py-4 [data-theme=light]_&:border-gray-200 [data-theme=light]_&:bg-gray-50">
              ...
              <p className="mt-2 text-sm leading-6 text-[#8892a4]">
                {displayCurrentPlayer === BLACK ? "흑" : "백"} 차례입니다.
                {isOnlinePlaying && isMyTurn && " 당신이 둘 수 있습니다."}
                {isOnlinePlaying && !isMyTurn && " 상대의 선택을 기다리는 중입니다."}
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-black/10 px-4 py-4 [data-theme=light]_&:border-gray-200 [data-theme=light]_&:bg-gray-50">
              ...
              <p className="mt-2 text-sm leading-6 text-[#8892a4]">
                {isCoarsePointer
                  ? "모바일에서는 유효한 칸을 한 번 터치해 미리보기한 뒤, 같은 칸을 다시 터치하면 수를 확정합니다."
                  : "마우스에서는 표시되는 유효 위치를 클릭하면 즉시 돌을 놓습니다."}
              </p>
            </div>
          </aside>
        </div>
