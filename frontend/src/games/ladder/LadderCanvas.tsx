...
export default function LadderCanvas({
  ...
  const scaledWidth = Math.round(width * Math.min(displayScale, 1.1));
  const scaledHeight = Math.round((height + 40) * Math.min(displayScale, 1.05));
  ...
  return (
    <div className="w-full overflow-x-auto pb-2">
      <svg
        ...
        style={{ minWidth: scaledWidth, maxHeight: scaledHeight }}
      >
        {/* Player names at top */}
        {players.map((name, i) => (
          <text
